...
class WorldDataHandler:
		...
		for user in users.get_user_list():
			with user.lock:
				...
				if user_password_sha1 != password_sha1:
					self.send("0020", user, "wloginfaild") #アカウント認証結果
					return
				if user.wlogin_client:
					user.wreset_login()
					self.send("0020", user, "wisonline") #アカウント認証結果
					return
				user.wreset_login()
				user.wlogin_client = self
				self.user = user
				self.send("0020", user, "wloginsucess") #アカウント認証結果
	...
	def do_0032(self, data_io):
		...
		self.send("0033", "world") #接続先通知要求(ログインサーバ/0032)の応答
...
WorldDataHandler.name_map = general.get_name_map(WorldDataHandler.__dict__, "do_")